import React from 'react'

const WaitforDriver = (props) => {
  return (
    <div className='flex flex-col px-9 py-3 gap-18'>
    <h5  onClick={()=>{
        props.setWaitforDriver(false)
      }} 
      className=' absolute text-4xl top-3 right-5'><i className="ri-arrow-down-s-line"></i></h5>

      <div>
         <img className='h-30 w-70' src="data:image/jpeg;base64," alt="" />
        <div className='flex flex-col gap-5 justify-between'>
            <div>
                <p className='text-2xl'>Driver name</p>
                <h2 className='text-2xl font-semibold'>Souvik Samanta</h2>
           </div>
            <div>
                <p className='text-2xl'>Car no:</p>
                 <h4 className='text-2xl font-semibold'>WB1234C3</h4>
            </div>
            <div>
                <p className='text-2xl'>Car name:</p>
            <h3 className='text-2xl font-semibold'>Harrier</h3>
            </div>

        </div>

      </div>
     
       <div className=''>
       <p className='text-2xl'>Pickup location</p>
       <p className='text-xl font-bold mt-5'>bally</p>
       </div>
     
     <div>
     <p className='text-2xl '>Destination</p>
     <p className='text-xl font-bold mt-5'>Belur</p>
     </div>
      
      <div>
      <h1 className='text-2xl'>Fare</h1>
      <p className='text-xl font-bold mt-5'>$7</p>

      </div>
      
       </div>
  )
}

export default WaitforDriver
